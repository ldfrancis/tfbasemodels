...
class ResNet(TFBaseModel):
    ...
    """
    def __init__(self):
        """Initialize a Resnet. sets number of filters in each block of each block and builds the model
        """
        self.preact = False
    ...
    def build_block(self, x, block=None, blocks_filters=[], shortcuts=[], strides=2):
        ...
        for i, block_filters, shortcut_ in enumerate(zip(blocks_filters, shortcuts)):
            reduction_block = len(blocks_filters)-1 if self.preact else 0
            strides_ = strides if i==reduction_block else 1
    ...
    def residual_block(self, x, shortcut="identity", bottleneck=True, block=None, block=None, conv_filters=[], strides=1):
        ...
        Returns:
            a Tensor
        """
        assert block != None and block != None
        assert shortcut in ["identity", "projection"]

        base_name = "residual_"+str(block)+"_"+str(block)
        ...
        if shortcut=="projection":
            ...
            shortcut_x = tf.keras.layers.MaxPool2D(pool_size=1, strides=strides, padding="same")(x) if strides > 1 else x

        # bottleneck
        if bottleneck:
            # first _1x1. this conv does spatial reduction with the strides and matches the shortcut spatial size
